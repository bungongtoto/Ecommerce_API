...
module.exports = (app) => {
    ...
    router.get('/', async (req, res, next) => {
        try {
            const response = await ProductServiceInstance.getAllByCategoryId(req.query.category_id);
    ...
    router.post('/',AuthorizationUtilsInstance.isAdmin, async (req, res, next) => {
        try {
            const data = req.body;
    ...
    router.put('/:product_id',AuthorizationUtilsInstance.isAdmin, async (req, res, next) => {
        try {
    ...
    router.post('/:product_id/images', AuthorizationUtilsInstance.isAdmin ,async (req, res, next) => {
        try {
            const {product_id} = req.params;
    ...
    router.put('/:product_id/images/:id', AuthorizationUtilsInstance.isAdmin ,async (req, res, next) => {
        try {
            const {product_id, id} = req.params;
    ...
    router.delete('/:product_id/images/:id',AuthorizationUtilsInstance.isAdmin, async (req, res, next) => {
        try {
            const {product_id, id} =  req.params
            const response = await ProductServiceInstance.deleteImage({product_id, id});
    ...
    router.get('/:product_id/reviews', async(req, res, next) => {
        try {
            const {product_id} = req.params;
            const response = await ProductServiceInstance.getAllReviewByProductId(product_id);
    ...
    // router.put('/categories',AuthorizationUtilsInstance.isAdmin ,async (req, res, next) => {
    //     try {
    //         const data = req.body;
