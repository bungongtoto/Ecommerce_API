...
module.exports = class ProductModel {
    ...
    async findOneById(id) {
        try {
            ...
            const statement =
                "SELECT p.id AS id, p.name AS name, p.unit_price AS unit_price, p.description AS description, p.quantity AS quantity, p.category_id AS category_id, COALESCE(json_agg(pi) FILTER (WHERE pi.id IS NOT NULL), '[]') AS images  FROM products p LEFT JOIN product_images pi ON p.id = pi.product_id WHERE p.id = $1 GROUP BY p.id, p.name, p.unit_price, p.description, p.quantity, p.category_id";
            const values = [id];
    ...
    async getAll() {
        try {
            ...
            const statement =
                "SELECT p.id AS id, p.name AS name, p.unit_price AS unit_price, p.description AS description, p.quantity AS quantity, p.category_id AS category_id, COALESCE(json_agg(pi) FILTER (WHERE pi.id IS NOT NULL), '[]') AS images  FROM products p LEFT JOIN product_images pi ON p.id = pi.product_id GROUP BY p.id, p.name, p.unit_price, p.description, p.quantity, p.category_id";
            const results = await db.query(statement);
    ...
    async getAllByCategoryId(category_id) {
        try {
            ...
            const statement =
                "SELECT p.id AS id, p.name AS name, p.unit_price AS unit_price, p.description AS description, p.quantity AS quantity, p.category_id AS category_id, COALESCE(json_agg(pi) FILTER (WHERE pi.id IS NOT NULL), '[]') AS images  FROM products p LEFT JOIN product_images pi ON p.id = pi.product_id WHERE p.category_id = $1 GROUP BY p.id, p.name, p.unit_price, p.description, p.quantity, p.category_id";
    ...
    async findReviewsByProductId(product_id) {
        try {
            const statement  = "SELECT pv.product_id AS product_id, pv.rating AS rating, pv.comment as comment, pv.timestamp as timestamp, json_build_object('user_id', pv.user_id, 'first_name', u.first_name, 'last_name',u.last_name) AS user FROM product_reviews pv LEFT JOIN users u ON pv.user_id = u.id WHERE pv.product_id = $1";
    ...
    async createReview(data) {
        try {
            const statement = pgp.helpers.insert(data, null, 'product_reviews') + 'RETURNING *';
    ...
    async updateReview(data) {
        try {
            ...
            const statement = pgp.helpers.update(params, null, 'product_reviews') + condition;
