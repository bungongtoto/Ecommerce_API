...
module.exports = class UserModel {
    ...
    async findOneById(id) {
        try {
            const statement = "SELECT users.id as id, users.email as email, users.telephone as telephone, roles.name as role FROM users JOIN roles ON users.role_id = roles.id WHERE users.id  = $1";
            const values = [id];
